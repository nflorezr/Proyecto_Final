...
void FILL()
{
	...
	//en esta funcion el objetivo es llenar la matriz que se volvera el plano para jugar, 
	la matriz sera llenada por los numeros que representaran los botones en el teclado//
		sum=1;
		for (i=2;i>=0;i--)
		{	
			for (j=0;j<=2;j++)
...
void trick()
{
// en esta funcion el objetivo es buscar si la posicion esta tomada y poner la X y el O respectivamente, 
se compara si la posicion es = al numero original puesta en la funcion FILL//
...
{ //Horizontales 
	...
	//se detecta cuando 3 puntos son iguales, en caso de que sean iguales (mismo simbolo) entonces ese jugador sera 
	marcado como el ganador transformando la variable win y enviandolo al main//
	...
	for (i=0;i<=2;i++)
{
 ...
 //si el juego no ha terminado despues de 8 rondas entonces nadie podra ganar, esto se detecta con un acumulador 
 que aumentara despues de cada ronda, si acu llega a 8 , el juego termina//
if (acu==8)
{
printf("Empate!");
win=1;
...
input();
trick();
//este while es utilizado para los casos en que un jugador intente poner su simbolo en una posicion que ya ha sido tomada, 
el while provocara un repetimiento infinito de esa iteracion //
...
//una vez que el win es transformado, el programa lo mirara y dira si alguien gano o no, en caso que win no sea transformado, 
el pograma continuara hasta que alguien gane o empate//
	if (win==1)
	{
		break;
	}
